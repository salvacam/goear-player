...
function notifyMe(cuerpo) {
	...
	if (!('Notification' in window)) {
		...
		var notification = new Notification('Goear Player', {'body': cuerpo, icon: img, sound: mp3, silent: true, tag: 'goer'});
		...
		Notification.requestPermission(function (permission) {
			if (permission === 'granted') {
				var notification = new Notification('Goear Player', {'body': cuerpo, icon: img, sound: mp3, silent: true, tag: 'goear'});
				notification.onclick = function(){
					navigator.mozApps.getSelf().onsuccess = function (){
						this.result.launch();
					}
...
function onBackKeyDown(e) {
	...
	if ( device.platform != "firefoxos" ) {	
		navigator.notification.confirm(
			...
			respuesta,            // callback to invoke with index of button pressed
			'Exit',           // title
...
//alert("5");
function respuesta(buttonIndex) {
    if ( buttonIndex == 1 ) {
		audio.pause();
		audio.src = '';
		audio = null;
